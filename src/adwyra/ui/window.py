...
class MainWindow(Adw.ApplicationWindow):
    ...
    def _build_prefs_page(self):
        ...
        hotkey_row.set_title("Назначить горячую клавишу")
        hotkey_row.set_subtitle("Откроет системные настройки клавиатуры")
        hotkey_row.set_activatable(True)
        hotkey_row.connect("activated", self._on_open_keyboard_settings)
        
        arrow = Gtk.Image.new_from_icon_name("go-next-symbolic")
        hotkey_row.add_suffix(arrow)
    ...
    def _on_open_keyboard_settings(self, row):
        import subprocess
        # Пробуем открыть настройки клавиатуры GNOME
        try:
            subprocess.Popen(["gnome-control-center", "keyboard"])
        except FileNotFoundError:
            # Fallback для других DE
            try:
                subprocess.Popen(["xdg-open", "gnome-control-center://keyboard"])
            except Exception:
                pass
